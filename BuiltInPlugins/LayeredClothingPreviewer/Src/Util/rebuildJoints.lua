--[[
	...
	manually to properly appear inside a Viewport Frame.
]]
...
local function buildJointTree(character)
	...
	local function createItems(dataItem)
		...
		local function findPairedJoints(dataItem)
			local childList = {}
			local jointList = {}
	...
	if dataItem.Joint then
		...
		if dataItem.Joint:IsA("Motor6D") then
			if isChildPart0 then
				dataItem.Part.CFrame =
					dataItem.Joint.Part1.CFrame * dataItem.Joint.C1 * dataItem.Joint.Transform * dataItem.Joint.C0:inverse()
			else
				dataItem.Part.CFrame =
					dataItem.Joint.Part0.CFrame * dataItem.Joint.C0 * dataItem.Joint.Transform * dataItem.Joint.C1:inverse()
			end
		else
			if isChildPart0 then
...
local function rebuildJoints(character)
	assert(character and character:FindFirstChild("Humanoid") and
		character:FindFirstChild("Humanoid").AutomaticScalingEnabled,
		"rebuildJoints() expects a character containing a Humanoid child with AutomaticScalingEnabled set to true")
