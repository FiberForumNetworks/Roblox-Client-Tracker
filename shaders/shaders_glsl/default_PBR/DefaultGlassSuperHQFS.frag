...
void main()
{
    ...
    vec3 f15 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f9 + CB2[3].w, 0.0, 1.0))) * f3.xyz) * (1.0 + (f8 * CB2[1].x))) * (texture2D(StudsMapTexture, f0).x * 2.0), f9).xyz;
    ...
    vec3 f28 = textureCube(EnvironmentMapTexture, reflect(-VARYING4.xyz, f16)).xyz;
    ...
    vec3 f59 = f58 * mix(vec3(1.0), VARYING2.xyz, vec3(f13));
    ...
    vec4 f61 = mix(mix(f60, vec4(mix((((((((vec3(1.0) - (f40 * (CB0[26].w * f1))) * 1.0) * CB0[10].xyz) * f33) + ((CB0[12].xyz * 1.0) * clamp(-f32, 0.0, 1.0))) * f25) + min(f22 + (CB0[8].xyz + (CB0[9].xyz * f23)), vec3(CB0[16].w))) * (f26 * f14), f29, vec3(mix(((1.0 - f11) * f1) * CB2[0].w, 1.0, VARYING7.w))), 1.0), vec4(f14)), vec4(f29, 1.0), vec4(((f43 * f43) * 0.800000011920928955078125) * f13)) + vec4((((f40 * ((f41 + (f41 * f41)) / (((f42 * f42) * ((f36 * 3.0) + 0.5)) * ((f35 * 0.75) + 0.25)))) * CB0[10].xyz) * f33) * f25, 0.0);
