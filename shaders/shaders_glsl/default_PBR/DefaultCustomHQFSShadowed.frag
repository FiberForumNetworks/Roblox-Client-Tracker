...
void main()
{
    ...
    vec3 f11 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f8 + CB2[3].w, 0.0, 1.0))) * f4.xyz) * (1.0 + (f7 * CB2[1].x))) * (texture2D(StudsMapTexture, f0).x * 2.0), f8).xyz;
    ...
    vec3 f35 = (((((((vec3(1.0) - (f32 * f20)) * (1.0 - f22)) * CB0[10].xyz) * f25) * f18) + min((f15.xyz * (f15.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f16.x)), vec3(CB0[16].w))) * f19) + ((((f32 * ((f33 + (f33 * f33)) / (((f34 * f34) * ((f28 * 3.0) + 0.5)) * ((f27 * 0.75) + 0.25)))) * CB0[10].xyz) * f25) * f18);
    vec4 f36 = vec4(f35.x, f35.y, f35.z, vec4(0.0).w);
    f36.w = f8;
    vec2 f37 = min(VARYING0.wz, VARYING1.wz);
    float f38 = min(f37.x, f37.y) / f1;
    vec3 f39 = mix(CB0[14].xyz, (sqrt(clamp((f36.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f38)) + f38, 0.0, 1.0)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    gl_FragData[0] = vec4(f39.x, f39.y, f39.z, f36.w);
}
