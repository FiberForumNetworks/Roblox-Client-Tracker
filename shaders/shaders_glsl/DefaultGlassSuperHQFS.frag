...
void main()
{
    ...
    float f8 = f7.x;
    float f9 = f3.w;
    vec4 f10 = mix(texture2D(SpecularMapTexture, f2 * CB2[2].w), texture2D(SpecularMapTexture, f2), vec4(clamp((f1 * CB2[4].z) - (CB2[3].z * CB2[4].z), 0.0, 1.0)));
    ...
    vec3 f14 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f9 + CB2[3].w, 0.0, 1.0))) * f3.xyz) * (1.0 + (f8 * CB2[1].x))) * (texture2D(StudsMapTexture, f0).x * 2.0), f9).xyz;
    ...
    vec4 f44 = texture2D(GBufferColorTexture, f38 + (f42 * clamp(min(texture2D(GBufferDepthTexture, f40 + f43).x * 500.0, texture2D(GBufferDepthTexture, f40 - f43).x * 500.0) - f37, 0.0, 1.0)));
    vec3 f45 = f44.xyz;
    vec3 f46 = ((f45 * f45) * CB0[15].x).xyz;
    vec3 f47 = f46 * mix(vec3(1.0), VARYING2.xyz, vec3(f13));
    ...
    vec4 f49 = mix(mix(f48, vec4(mix((min(f23 + (CB0[8].xyz + (CB0[9].xyz * f24)), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f17, 0.0, 1.0)) + (CB0[12].xyz * max(-f17, 0.0))) * f26)) * f27, f30, vec3(mix((f10.y * f1) * CB2[0].w, 1.0, VARYING7.w))), 1.0), vec4(clamp((f12 - 1.0) + f9, 0.0, 1.0))), vec4(f30, 1.0), vec4(((f31 * f31) * 0.800000011920928955078125) * f13)) + vec4(CB0[10].xyz * ((((step(0.0, f17) * mix(f11.x, CB2[0].y, VARYING7.w)) * f26) * pow(clamp(dot(f15, normalize(f16 + f28)), 0.0, 1.0), mix(f11.y, CB2[0].z, VARYING7.w))) * f13), 0.0);
