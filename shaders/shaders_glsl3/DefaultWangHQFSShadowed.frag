...
void main()
{
    ...
    vec3 f48 = ((min((f25.xyz * (f25.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f26.x)), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f20, 0.0, 1.0)) + (CB0[12].xyz * clamp(-f20, 0.0, 1.0))) * f46)) * mix((f21 * f21).xyz, (f47 * f47) * CB0[15].x, vec3((f16.y * f2) * CB2[0].w)).xyz) + (CB0[10].xyz * (((step(0.0, f20) * f17.x) * f46) * pow(clamp(dot(f18, normalize(f19 + normalize(VARYING4.xyz))), 0.0, 1.0), f17.y)));
