...
void main()
{
    ...
    vec4 f51 = mix(vec4(mix((min(f24 + (CB0[8].xyz + (CB0[9].xyz * f25)), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f18, 0.0, 1.0)) + (CB0[12].xyz * max(-f18, 0.0))) * f45)) * f46, f49, vec3(mix((f10.y * f1) * CB2[0].w, 1.0, VARYING7.w))) * f14, f14), vec4(f49, 1.0), vec4(((f50 * f50) * 0.800000011920928955078125) * f13)) + vec4(CB0[10].xyz * ((((step(0.0, f18) * mix(f11.x, CB2[0].y, VARYING7.w)) * f45) * pow(clamp(dot(f16, normalize(f17 + f47)), 0.0, 1.0), mix(f11.y, CB2[0].z, VARYING7.w))) * f13), 0.0);
