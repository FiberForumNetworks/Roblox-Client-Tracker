...
void main()
{
    ...
    vec2 f6 = (f5.xy + (((texture(NormalDetailMapTexture, f3 * CB2[1].w).xyz * 2.0) - vec3(1.0)).xy * CB2[2].x)).xy * f2;
    float f7 = f6.x;
    float f8 = f4.w;
    vec2 f9 = mix(vec2(CB2[1].y, CB2[1].z), (texture(SpecularMapTexture, f3).xy * vec2(CB2[0].y, CB2[0].z)) + vec2(0.0, 0.00999999977648258209228515625), vec2(f2));
    ...
    vec3 f13 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f8 + CB2[3].w, 0.0, 1.0))) * f4.xyz) * (1.0 + (f7 * CB2[1].x))) * (texture(StudsMapTexture, f0).x * 2.0), f8).xyz;
    ...
    vec3 f39 = ((min((f17.xyz * (f17.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f18.x)), vec3(CB0[16].w)) + (((CB0[10].xyz * clamp(f12, 0.0, 1.0)) + (CB0[12].xyz * clamp(-f12, 0.0, 1.0))) * f38)) * (f13 * f13).xyz) + (CB0[10].xyz * (((step(0.0, f12) * f9.x) * f38) * pow(clamp(dot(f10, normalize(f11 + normalize(VARYING4.xyz))), 0.0, 1.0), f9.y)));
    vec4 f40 = vec4(f39.x, f39.y, f39.z, vec4(0.0).w);
    f40.w = f8;
    vec2 f41 = min(VARYING0.wz, VARYING1.wz);
    float f42 = min(f41.x, f41.y) / f1;
    vec3 f43 = mix(CB0[14].xyz, (sqrt(clamp((f40.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f42)) + f42, 0.0, 1.0)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    _entryPointOutput = vec4(f43.x, f43.y, f43.z, f40.w);
}
