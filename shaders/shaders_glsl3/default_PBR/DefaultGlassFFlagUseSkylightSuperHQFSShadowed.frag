...
void main()
{
    ...
    vec3 f77 = f76 * mix(vec3(1.0), VARYING2.xyz, vec3(f13));
    ...
    vec4 f79 = mix(mix(f78, vec4(mix(((((((vec3(1.0) - (f58 * (CB0[26].w * f1))) * 1.0) * CB0[10].xyz) * f51) * f45) + min(f24 + (CB0[8].xyz + (CB0[9].xyz * f25)), vec3(CB0[16].w))) * (f46 * f14), f49, vec3(mix(((1.0 - f11) * f1) * CB2[0].w, 1.0, VARYING7.w))), 1.0), vec4(f14)), vec4(f49, 1.0), vec4(((f61 * f61) * 0.800000011920928955078125) * f13)) + vec4((((f58 * ((f59 + (f59 * f59)) / (((f60 * f60) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25)))) * CB0[10].xyz) * f51) * f45, 0.0);
