...
void main()
{
    ...
    vec4 f61 = mix(vec4(mix(((((((vec3(1.0) - (f57 * (CB0[26].w * f1))) * 1.0) * CB0[10].xyz) * f50) * f44) + min(f23 + (CB0[8].xyz + (CB0[9].xyz * f24)), vec3(CB0[16].w))) * (f45 * f13), f48, vec3(mix(((1.0 - f11) * f1) * CB2[0].w, 1.0, VARYING7.w))) * f13, f13), vec4(f48, 1.0), vec4(((f60 * f60) * 0.800000011920928955078125) * clamp(f12, 0.0, 1.0))) + vec4((((f57 * ((f58 + (f58 * f58)) / (((f59 * f59) * ((f53 * 3.0) + 0.5)) * ((f52 * 0.75) + 0.25)))) * CB0[10].xyz) * f50) * f44, 0.0);
