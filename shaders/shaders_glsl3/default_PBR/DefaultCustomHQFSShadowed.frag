...
void main()
{
    ...
    float f41 = max(f9.y * CB2[0].z, 0.04500000178813934326171875);
    float f42 = (f9.x * CB2[0].y) * f40;
    ...
    vec3 f54 = (((((((vec3(1.0) - (f51 * f40)) * (1.0 - f42)) * CB0[10].xyz) * f44) * f38) + min((f17.xyz * (f17.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f18.x)), vec3(CB0[16].w))) * f39) + ((((f51 * ((f52 + (f52 * f52)) / (((f53 * f53) * ((f47 * 3.0) + 0.5)) * ((f46 * 0.75) + 0.25)))) * CB0[10].xyz) * f44) * f38);
    vec4 f55 = vec4(f54.x, f54.y, f54.z, vec4(0.0).w);
    f55.w = f8;
    vec2 f56 = min(VARYING0.wz, VARYING1.wz);
    float f57 = min(f56.x, f56.y) / f1;
    vec3 f58 = mix(CB0[14].xyz, (sqrt(clamp((f55.xyz * clamp((clamp((f1 * CB0[24].x) + CB0[24].y, 0.0, 1.0) * (1.5 - f57)) + f57, 0.0, 1.0)).xyz * CB0[15].y, vec3(0.0), vec3(1.0))) + vec3((-0.00048828125) + (0.0009765625 * fract(52.98291778564453125 * fract(dot(gl_FragCoord.xy, vec2(0.067110560834407806396484375, 0.005837149918079376220703125))))))).xyz, vec3(clamp((CB0[13].x * length(VARYING4.xyz)) + CB0[13].y, 0.0, 1.0)));
    _entryPointOutput = vec4(f58.x, f58.y, f58.z, f55.w);
}
