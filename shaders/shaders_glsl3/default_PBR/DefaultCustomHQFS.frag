...
void main()
{
    ...
    vec3 f11 = vec4(((mix(vec3(1.0), VARYING2.xyz, vec3(clamp(f8 + CB2[3].w, 0.0, 1.0))) * f4.xyz) * (1.0 + (f7 * CB2[1].x))) * (texture(StudsMapTexture, f0).x * 2.0), f8).xyz;
    ...
    vec4 f17 = texture(ShadowMapTexture, VARYING7.xy);
    ...
    float f21 = max(f9.y * CB2[0].z, 0.04500000178813934326171875);
    float f22 = (f9.x * CB2[0].y) * f20;
    vec3 f23 = -CB0[11].xyz;
    ...
    float f36 = 1.0 - f22;
    vec3 f37 = ((((((((vec3(1.0) - (f33 * f20)) * f36) * CB0[10].xyz) * f26) + ((CB0[12].xyz * f36) * clamp(-f25, 0.0, 1.0))) * f18) + min((f15.xyz * (f15.w * 120.0)).xyz + (CB0[8].xyz + (CB0[9].xyz * f16.x)), vec3(CB0[16].w))) * f19) + ((((f33 * ((f34 + (f34 * f34)) / (((f35 * f35) * ((f29 * 3.0) + 0.5)) * ((f28 * 0.75) + 0.25)))) * CB0[10].xyz) * f26) * f18);
    ...
    _entryPointOutput = vec4(f41.x, f41.y, f41.z, f38.w);
}
